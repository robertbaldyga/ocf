...
struct ocf_prefetch_stream {
	env_atomic stream_len_db[MAX_COUNT][MAX_DB_SECTORS];
	struct ocf_ohash64 ohash64_handle;
	uint32_t req_cnt;
};
...
/* ===========================================================================*/
static inline void clear_stat_on_need(struct ocf_prefetch_stream *pf)
{
...
static uint64_t get_ohash_item(struct ocf_ohash64 *hash,
...
{
	struct ocf_prefetch_stream *pf = (struct ocf_prefetch_stream *)
			core->prefetch_handles[ocf_prefetch_stream];
	uint i;
	ohash_t item;
	struct ocf_ohash64 *hash = NULL;
	uint64_t sector;
	...
	if (sec_to_pf > item.pfcnt) {
		...
		req_info->pf_id = ocf_prefetch_stream;
...
{
	struct ocf_prefetch_stream *pf = NULL;
	int ret;

	pf = env_aligned_zalloc(ENV_PROCESSOR_CACHE_LINE_SIZE,
			sizeof(struct ocf_prefetch_stream));
	if (!pf)
		return -OCF_ERR_NO_MEM;

	ret = ocf_ohash_create(core, &pf->ohash64_handle, OHASH_SIZE,
			"prefetch");
	if (ret) {
		env_aligned_free(pf);
		return ret;
	...
	core->prefetch_handles[ocf_prefetch_stream] = pf;
...
void ocf_pf_stream_destroy(ocf_core_t core)
{
	...
	core->prefetch_handles[ocf_prefetch_stream] = NULL;
